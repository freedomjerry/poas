...
path = os.getcwd()
logging.config.fileConfig("step5_LDA/logging.conf")
logger = logging.getLogger()
# loggerInfo = logging.getLogger("TimeInfoLogger")
# Consolelogger = logging.getLogger("ConsoleLogger")

conf = configparser.ConfigParser()
conf.read("step5_LDA\setting.conf") 
...
class LDAModel(object):
    ...
    def _phi(self):
        for i in range(self.K):
            self.phi[i] = (self.nw[i] + self.beta)/(self.nwsum[i]+self.dpre.words_count * self.beta)
        ...
        with codecs.open(self.tassginfile,'w') as f:
            ...
            for x in range(self.dpre.docs_count):
                if self.Z[x]:
                    ...
                    cursor = db.cursor()
                    sql = "UPDATE comments_{keyword} SET label='{values}' WHERE id='{t}'".format(keyword=keyword, values=label,t=x+1)
                    try:
...
def f_testset_word_count(trainfile):                                     #测试集的词数统计
...
def run(keyword):
    trainfile1 = "data_full_{keyword}.dat".format(keyword = keyword)
    trainfile = trainfile1
    dpre = preprocessing(trainfile1)
    lda = LDAModel(dpre)
    lda.est(keyword)
