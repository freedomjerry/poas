...
def wordcloud(keyword):
    ...
    f=open('data_keywords_{keyword}.dat'.format(keyword = keyword),'r')
    ...
    image= Image.open('step3 wordshow/background.png')
    graph = np.array(image)
    wc = WordCloud(font_path='step6_vision/vision/Post/main/Songti.ttc',background_color='White',max_words=50,mask=graph)
    ...
    plt.axis("off")
    plt.show()
    wc.to_file('{keyword}.png'.format(keyword = keyword))
    ...
    plt.savefig("{keyword}.jpg".format(keyword = keyword),dpi = 360)
    plt.show()